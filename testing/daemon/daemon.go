...
// Cancelling the context will kill the daemon(s).
// Returns the log capture for the daemon, along with the filesystem path for the wallets.
func New(ctx context.Context, path string, id uint64, port int) (*logger.LogCapture, string, error) {
	...
	wallet1Accounts := [][]byte{
		// These are the well-known interop keys, starting from index 0.
	...
	for i := 0; i < len(wallet1Accounts); i++ {
	...
	for i := 0; i < len(wallet2Accounts); i++ {
	...
	peersMap := map[uint64]string{
		10101: fmt.Sprintf("localhost:%d", port),
	}
	...
	sender, err := sendergrpc.New(ctx,
		...
		sendergrpc.WithCACert([]byte(resources.CACrt)),
	)
	if err != nil {
	...
	_, err = grpcapi.New(ctx,
		...
		grpcapi.WithCACert([]byte(resources.CACrt)),
