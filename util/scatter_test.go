...
func TestDouble(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			workerResults, err := util.Scatter(len(inValues), func(offset int, entries int, _ *sync.RWMutex) (any, error) {
...
func TestMutex(t *testing.T) {
	totalRuns := 1048576
	val := 0
	_, err := util.Scatter(totalRuns, func(offset int, entries int, mu *sync.RWMutex) (any, error) {
		for i := 0; i < entries; i++ {
			mu.Lock()
			val++
			mu.Unlock()
